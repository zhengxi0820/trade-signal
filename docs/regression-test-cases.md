...
| R-20260901-06 | 「仅看涨」勾选框交互 | 已登录 + 点过查询 + 截止=历史周期 | 选历史截止→卡片头「列设置」左侧出现勾选框；勾选→列表与计数只留 nextClose>close；切回最新周期→勾选框隐藏且过滤失效 | 显隐正确、两框独立、计数联动、样式与现有勾选框一致 | 静态资源生效确认（线上首页含 bull-check ×2）；交互点击验证被浏览器自动化环境事件注入阻断（2026-09-01）；**2026-09-21 本地发版 jar 点验通过**：2026/06/15 周有区分度样本（601005 close=1.23/nextClose=1.14），勾选 1→0 计数联动、取消恢复，详见 R-20260921-10 | 通过（2026-09-21 点验关闭） |
...
## 2026-09-21 补充用例（图表口径快照修复 + 「展示历史周期/展示最新周期」两项开关）

改动：纯前端（index.html / js/app.js，静态资源 v=20）。①图表口径快照 `qSnapshot`：`loadLists()` 发起查询时固化本次查询的完整查询串/kdjType/n,m1,m2/截止与最新标签/isLatest/最新周期对象；点股票后的 series 取数（`loadChart`/`loadLatestChart`）、横轴 `periodLabel`、KDJ 标题、卡片标签（`chartPeriodName`/`chartCutoffLabel`/`chartLatestLabel`）、「仅看涨」基准（`chartIsLatest`）一律读快照——修复「查询后改周期不点查询，点股票图表按新周期取数」缺陷；②参数设置「查询历史周期时同步展示最新周期」单项开关拆为「展示历史周期」「展示最新周期」两项独立开关（真值表：截止=最新一律单图；历史截止按两项组合出 0/1/2 张图；均关时点股票只做行高亮不出卡片）；`historyChartVisible = showHistoryChart || (showLatestChart && chartIsLatest)`、`latestChartVisible = showLatestChart && !chartIsLatest`，切换经 `refreshChartPanes` 即时补拉/销毁；③localStorage：新增 `ts_show_history_chart`（默认开），`ts_show_latest_chart` 沿用（存量记忆无缝迁移）。

| 编号 | 用例 | 前置 | 步骤 | 预期 | 实测 | 状态 |
|---|---|---|---|---|---|---|
| R-20260921-01 | 快照口径-图表取数（原缺陷场景） | 本地 dev 服务 + 浏览器密钥登录；周线 | 周线查询历史周 07/20~07/24 → 截止改回最新周 07/27~07/31 **不点查询** → 点金叉股票 | 卡片「截止周期」= 07/20~07/24；series 请求 tradeDateMin=20260720&tradeDateMax=20260724；最新周期图截止=07/27~07/31（快照内的最新周） | 卡片标签 07/20~07/24；拦截到的请求 `...&tradeDateMin=20260720&tradeDateMax=20260724&code=300750`，最新图请求 `tradeDateMin=20260727...` | 通过 |
| R-20260921-02 | 快照口径-仅看涨显隐 | 同 R-01 场景 | 历史周查询后观察仅看涨勾选框；截止改最新周不查询再观察；重新点查询后观察 | 历史周查询后可见；改截止不查询**不消失**（跟随快照）；重新查询（截止=最新）后隐藏 | 三步实测：2 可见 → 2 可见 → 0 可见 | 通过 |
| R-20260921-03 | 两项开关组合（历史截止） | 历史截止 + 选中股票 + 参数设置面板 | 依次切换 (开,开)/(开,关)/(关,开)/(关,关) 并读 DOM 图表显隐 | 双图 / 仅截止图 / 仅最新图 / 整卡消失 | (关,开)=card T cut F latest T；(开,关)=T T F；(关,关)=全 F；(开,开)=T T T，全部符合 | 通过 |
| R-20260921-04 | 截止=最新一律单图 | 两开关全开 | 查询最新周 07/27~07/31（有选中股票自动刷新） | 仅一张截止图、无最新周期区块；标签=07/27~07/31；仅看涨隐藏 | cut=T、`#kdjChartLatest` 不在 DOM、cutoffLabel=07/27~07/31、bull=0 | 通过 |
| R-20260921-05 | 开关切换即时生效 | 历史截止 + 选中股票 | 开「展示最新周期」 | 可见但未渲染的图即时补拉；不可见的即时销毁；不重复取已在屏的图 | 开→最新图即时补拉渲染，截止图保持不重取 | 通过 |
| R-20260921-06 | localStorage 存量迁移 | 预置 `ts_show_latest_chart='0'`、无 `ts_show_history_chart` 后刷新 | 读两项开关状态 | 展示最新周期=关（沿用旧记忆）、展示历史周期=开（新项默认） | 实测 历史=true / 最新=false | 通过 |
| R-20260921-07 | 「点击查询后关闭图表」回归 | 该开关开 | 查询 → 点股票 → 再查询 | 选中后卡片出现；再次查询后整卡关闭（语义不变） | card true → false | 通过 |
| R-20260921-08 | 自动化测试套件 | 本地构建环境 | ./mvnw test | 全绿（纯前端改动，用例数不变） | 69/69 通过 | 通过 |
| R-20260921-09 | 双图布局视觉点验 | 历史截止 + 两开关全开 + 选中股票 | 截图核对：截止图 + 分隔线 + 最新周期区块、KDJ(9,3,3) 标题、交汇点标注 | 两图纵排、标签正确、渲染无异常 | 截图确认：截止 07/20~07/24 与最新 07/27~07/31 双图渲染正常 | 通过 |
| R-20260921-10 | 「仅看涨」勾选交互点验（关 2026-09-01 遗留待办） | 本地发版 jar（devkey-12345）+ 周线历史截止 | 选 2026/06/15~06/18 周（有区分度：601005 close=1.23 > nextClose=1.14）→ 查询 → 勾选金叉表「仅看涨」→ 取消 | 勾选后列表与计数过滤为 0；取消后恢复原行 | 勾选前（1）601005 → 勾选后（0）0 行 → 取消后（1）601005 恢复 | 通过 |
| R-20260921-11 | 线上发版冒烟（2026-09-21 21:08 重启） | jar 已替换、systemd active | 公网 https://zhengxi.online 与服务器本机 8080 各取 / 与 /js/app.js?v=20 比对；密钥登录；periods/series/all-stocks；journalctl 查错 | 公网=本机新版（v=20，qSnapshot/showHistoryChart 特征命中）；登录 204；三个接口 200；无 ERROR/Exception | 公网 index 19745 字节与服务器本机完全一致；app.js v20 qSnapshot×10/showHistoryChart×3；login 204、periods 200、series(600519) 200、all-stocks 200；新进程 PID 3751361 2.5s 启动无错误。注：本机侧管道 `curl \| grep` 对该站点间歇性 SSL 失败（exit 35）造成假 0，落盘比对为准 | 通过 |
