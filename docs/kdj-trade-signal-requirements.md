...
- 进行中的周期（如本周三查本周，其最后一个计划交易日的数据尚未获取）**不参与** KDJ 计算与
  信号判断，避免信号随盘中价格闪烁。
- 每股物化口径：周期完结后，**该股在周期内 ≥1 个交易日即物化该周期 bar**（首/末交易日 = 该股
  真实首/末交易日，停牌股不丢 K 线）；数据后补导致的旧部分行由"窗口内先删后插重建 + 全历史
  对账"收敛，同一周期绝不出现两行。
...
### 4.3 「仅看涨」筛选（前端，基于出参 nextClose）

针对**截止周期为非最新周期**的场景（回看历史信号）：金叉、交易位两个面板各有一个独立勾选框「仅看涨」，勾选后只显示「下一周期收盘价 > 截止周期收盘价」的股票，面板计数随之联动。纯前端过滤，不新增请求与入参；数据来自扫描出参 `nextClose` 字段（口径见接口文档）。

判定口径（权威）：

- 「这一周期」= 用户选的截止周期，即出参 `close`。
- 「下一周期」= **全局日历上截止周期的下一期**（work_day 日历推导），取该期该股的已完结 bar 收盘价，与查询同复权口径；**严格大于**才入选。
- 下一期该股**整期停牌**（无 bar，第一根 bar 跳到更后面的周期）→ `nextClose = null`，勾选时排除。例：截止 x-2、x-1 整期停牌、x 期复牌且收盘价更高——不入选（下一期严格是 x-1，x 不是）。下一期部分停牌（该期 ≥1 个交易日有成交）不受影响。
- 截止即最新周期时没有下一期 → 勾选框隐藏、过滤失效。
