...
> **本文件已停更**（最后条目 2026-08-16）：2026-08-19 起的发版记录改记于
> `docs/trade-signal-deployment.md`「已完成里程碑」+ `docs/regression-test-cases.md`（回归用例），
> 不再在此重复。以下为历史条目存档。
...
## 2026-08-16 — 物化准确性修复：窗口对齐 + 先删后插 + 全量重建对账

### 背景

- 发现物化表存在脏数据：2026-08-14 增量物化窗口 cutoff（07-24）落在 07-20 周中间，
  把全市场 5538 只 × adjust 0/1 = 11,040 行的"07-20 周"截断成 07-24 单日 bar 并覆盖正确行；
- 上工申贝（600843）因该污染行 K/D 递推被带偏，金叉从 08-03 周"平移"到 08-10 周，被误筛进最新周金叉/交易位列表；
- 另有 600635 等"数据后补后旧部分行残留"（同一周期两条 bar）问题。

### 改动

- `scripts/aggregate/period_bar.py`：
  - 增量窗口起点对齐周期第一天（周→周一、月→1号、季→季首），杜绝边界周期截断；
  - 写入从 upsert 改为"窗口内先删后插"（同事务提交）：正常股删 `PERIOD_END >= 对齐窗口起点` 的行，
    除权事件股删该股该类型全部行，再插入本次聚合结果——数据后补不残留旧行、同一周期绝不两行；
  - `--full` 全量重建同样先删该类型全部行再流式插入；
  - 停牌股周期内 ≥1 交易日即物化（不丢 K 线），首/末交易日 = 该股真实首/末交易日。
- 文档：需求 §2.4、管线文档、回归用例 R-20260815-13~17。

### 验证

- 本地：py_compile 通过；`_align_window_start` 8 个边界用例全过。
- 线上（2026-08-16 00:05~02:09）：`--full` 全量重建 rc=0（7404s），
  周 7,097,114 / 月 1,684,600 / 季 566,372 行，唯一键重复行=0；
  07-20 周恢复为 11,038 行完整周（600843/300237/300176 对拍一致），
  600635 旧残留清除，920238 首周单日行为合法新股数据；
  清缓存后最新周金叉列表 483→412 只、交易位 11→8 只，均不含 600843（详见 R-20260815-13~17）。
