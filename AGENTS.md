...
- `handler/KDJHandler.java` — 全部核心计算（聚合、KDJ 递推、交叉点、交易位判断），**纯函数，不依赖 Spring 与数据库**。例外：全市场扫描的周/月/季线读物化表 `stock_period_bar`（scripts 周频物化，口径 = `KDJHandler.aggregate`，`KDJScanWindowCacheTest` 对拍）；单票序列与日线扫描仍走 Java 侧
...
| `docs/trade-signal-schema.sql` | 数据库表结构（stock_quote / stock_quote_log / stock_info / stock_dividend / work_day / stock_period_bar / app_user / user_watchlist），与 orm/entity 一一对应 |
...
| `docs/trade-signal-handoff.md` | 交接手册：新 agent/新人入场第一课（系统全貌、当前状态、踩坑速查、待办） |
