...
| 编号 | 用例 | 前置 | 步骤 | 预期 | 实测 | 状态 |
|---|---|---|---|---|---|---|
| R-20260815-13 | 窗口对齐 | cutoff 落在周期中间（如周四） | `_align_window_start` + 增量物化 | 边界周完整重算，PERIOD_START/END=真实首末交易日，不再截断 | 单元用例 8/8 通过；线上重建后 07-20 周恢复完整 | 通过 |
| R-20260815-14 | 先删后插防残留 | 数据后补（600635 类） | 增量物化 | 旧部分行被删除，同一周期仅一行 | 待线上重建后确认 | 待确认 |
| R-20260815-15 | 全量重建对账 | 服务器新脚本 | `--full` 重建 | 全表与 stock_quote 聚合一致；无重复行 | 待线上验证 | 待确认 |
| R-20260815-16 | 上工申贝回归 | 重建完成 | 最新周金叉/交易位扫描 | 600843 不在列表；其 K/D 与序列接口一致 | 待线上验证 | 待确认 |
| R-20260815-17 | 全表一致性 | 重建完成 | 重复行计数 + 单日异常行计数 + 抽样对拍 | 重复=0；07-20 周单日行=0；抽样差异=0 | 待线上验证 | 待确认 |
