...
- 2026-09-21 图表口径快照修复 + 「展示历史周期/展示最新周期」两项开关发版（commit b98ccde，纯前端静态资源 v=20）：①`loadLists` 发起查询时固化 `qSnapshot`，点股票后的图表取数（`loadChart`/`loadLatestChart`）、横轴/KDJ 标题、卡片标签、最新周期图截止、「仅看涨」基准（`chartIsLatest`）一律按查询快照——修复「查询后改周期不点查询，点股票图表按新周期取数」缺陷；②「查询历史周期时同步展示最新周期」单项开关拆为「展示历史周期」「展示最新周期」两项（截止=最新一律单图；历史截止按组合出 0/1/2 张图；均关不出卡片、点股票只行高亮），切换经 `refreshChartPanes` 即时补拉/销毁；③localStorage 新增 `ts_show_history_chart`（默认开），`ts_show_latest_chart` 存量记忆无缝沿用。回归 69/69 + 浏览器点验（回归库 R-20260921-01~09）；线上冒烟：公网与服务器本机页面逐字节一致（v=20 特征命中）、登录 204、periods/series/all-stocks 200、新进程 2.5s 启动无错误（R-20260921-11）；仅看涨勾选交互点验通过（R-20260921-10），2026-09-01 遗留待办关闭。

- 2026-09-17 信号限制项五项开关 + 前端 localStorage 记忆发版：①五个信号限制项（上次/当前金叉交汇上限、死叉交汇上限、金叉最小/最大间距）各加 `xxxEnabled` 开关（"1"=生效默认、"0"=停用该项，未传等同 "1"，老调用方行为不变）；开关参与结果缓存 key；currGoldCrossMaxEnabled=0 同时停用 series/gold-cross/all-stocks 的金叉标注过滤；②前端五项各加 el-switch（关=输入框置灰）+ `ts_query_params`（整个参数面板）/`ts_ui_state`（仅看涨/列设置/板块筛选）两组 localStorage 记忆（刷新/重开浏览器保留、清缓存回默认，周期截止不记忆保持自动最新）。回归 69/69 全绿（回归库 R-20260917-01~08）；线上冒烟：全开 16 只 vs 逐开关关闭 16~51 只（只增不减、子集成立）、非法开关值 400、同参缓存重试 2.3ms、静态资源新版生效。浏览器点验（2026-09-18 用户实测）：开关点击交互与刷新后 localStorage 记忆恢复均正常，R-05/06 关闭。

- 2026-09-01 查询触发制补全 + 「仅看涨」筛选发版：①前端初始加载/登录成功不再自动发全市场扫描请求（`initPeriods(true)` + 待查橙点 + 未查询空态引导；`/kdj/periods`、`/watchlist` 保持自动加载）；②金叉/交易位出参 `CrossStockVO` 新增 `nextClose`（严格下一期口径：全局日历上截止的下一期该股收盘价，同复权；整期停牌跳期置 null；截止=最新周期 null），三条扫描取数路径 peek 下一根 bar，历史截止批量兜底 SQL 上界精确放宽到下一期期末（periodCalendar 边界，每股至多多读一根）；③金叉/交易位卡片头「列设置」左侧各一个独立「仅看涨」勾选框（截止=最新隐藏），纯前端过滤 `nextClose>close`、计数联动。回归 60/60 全绿（回归库 R-20260901-01~06）；线上冒烟：周线历史截止 nextClose=1253.00 与库一致、默认周期 null、首页新静态资源生效、新进程启动无错误（旧进程优雅停机期的 NoClassDefFoundError 为 jar 替换固有现象，无碍）。遗留：勾选框交互人工点验（浏览器自动化事件注入故障未点完）。
